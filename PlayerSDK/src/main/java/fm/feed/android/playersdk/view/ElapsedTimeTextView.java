...
public class ElapsedTimeTextView extends TextView implements NavListener {

    private static final String TAG = ElapsedTimeTextView.class.getSimpleName();

    private Player mPlayer;
    private String mTextForNoTime = "";

    public ElapsedTimeTextView(Context context) {
        super(context);

        init(null);
    }

    public ElapsedTimeTextView(Context context, AttributeSet attrs) {
        super(context, attrs);

        init(attrs);
    }

    public ElapsedTimeTextView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void onProgressUpdate(Play play, int elapsedTime, int totalTime) {
        setText(TimeUtils.toProgressFormat(elapsedTime));
        setContentDescription(TimeUtils.toProgressAccessibilityFormat(elapsedTime));
    }
}
