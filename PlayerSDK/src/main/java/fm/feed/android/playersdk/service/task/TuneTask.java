...
/**
 * This task asks the Feed.fm server for a new 'play'. When the play is retrieved,
 * the URL for the audio file is passed to a MediaPlayer object and we wait for
 * the MediaPlayer to be in a 'prepared' state. When that happens, the MediaPlayer
 * instance is thrown into the MediaPlayerPool and we're done.
 *
 * If there are connectivity problems trying to get the MediaPlayer prepared, this
 * task will re-queue itself in the TaskQueueManager to basically try again. If
 * there are API or Unknown errors, they'll be passed to the provided listeners.
 * Since this is a 'SkippableTask', the standard way of handling API or unknown
 * errors is to skip this task.
 *
